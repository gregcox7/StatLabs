...
```{r}
tetris_data %>%
    mutate(effect = intrusions_post - intrusions_pre)
```
...
lm(effect ~ condition, data = tetris_data_effect) %>%
    anova()
```
...
with(
    tetris_data %>%
        mutate(effect = intrusions_post - intrusions_pre),
    pairwise.t.test(x = effect, g = condition, p.adjust.method = "bonferroni")
)
```
