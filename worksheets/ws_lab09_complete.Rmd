...
c(sample_mean - critical_t * standard_error, sample_mean + critical_t * standard_error)
```
...
t_score <- (sample_mean - null_value) / standard_error

pt(q = t_score, df = degrees_of_freedom, lower.tail = FALSE)
```
...
lullaby_null_dist %>%
    visualize() +
    shade_p_value(obs_stat = lullaby_obs_t, direction = "two-sided")
```
...
```{r}
lullaby_null_dist %>%
    get_p_value(obs_stat = lullaby_obs_t, direction = "two-sided")
```
