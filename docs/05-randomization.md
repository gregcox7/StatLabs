...
In the third line, we `specify` which variable is the **explanatory variable** (`prev_shot`) and which variable is the **response variable** (`shot`).  Because the response variable is a categorical variable, we also need to tell R that a "success" is a Hit (or "H").
...
Note that your simulation will probably not look the same as those shown here because they were shuffled *randomly*.
